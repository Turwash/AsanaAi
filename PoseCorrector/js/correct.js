...
function setup() {
  ...
  if ("speechSynthesis" in window) {
    ...
    if (voices.length) {
      // set voice to Heera if available
  ...
  if (pose) {
    ...
    setInterval(() => {
      ...
      if (selected_pose != "none") {
        // if a pose is selected then only correct
        let leftKHH_range,
          rightKHH_range,
          leftWEH_range,
          rightWEH_range,
        ...
        switch (selected_pose) {
          case "MOUNTAIN":
            // leftKHH_range = in_range(90, angles.left.KHH, 95); // Adjusting angle ranges
            // rightKHH_range = in_range(90, angles.right.KHH, 95); // Adjusting angle ranges
            // leftWEH_range = in_range(20, angles.left.WEH, 40); // Adjusting angle ranges
            // rightWEH_range = in_range(20, angles.right.WEH, 40); // Adjusting angle ranges
            // leftSEW_range = in_range(150, angles.left.SEW, 180); // Adjusting angle ranges
            // rightSEW_range = in_range(150, angles.right.SEW, 180); // Adjusting angle ranges
            // leftAKH_range = in_range(140, angles.left.AKH, 230); // Adjusting angle ranges
            // rightAKH_range = in_range(140, angles.right.AKH, 230); // Adjusting angle ranges

            leftKHH_range = in_range(85, angles.left.KHH, 95); // Widen the acceptable range for left KHH
            rightKHH_range = in_range(85, angles.right.KHH, 95); // Widen the acceptable range for right KHH

            leftWEH_range = in_range(15, angles.left.WEH, 40); // Widen the acceptable range for left WEH
            rightWEH_range = in_range(15, angles.right.WEH, 40); // Widen the acceptable range for right WEH

            leftSEW_range = in_range(145, angles.left.SEW, 180); // Widen the acceptable range for left SEW
            rightSEW_range = in_range(145, angles.right.SEW, 180); // Widen the acceptable range for right SEW

            leftAKH_range = in_range(135, angles.left.AKH, 230); // Significantly widen the acceptable range for left AKH
            rightAKH_range = in_range(135, angles.right.AKH, 230);


            if (
              (leftKHH_range === 0 || rightKHH_range === 0) &&
              (leftWEH_range === 0 || rightWEH_range === 0) &&
              (leftSEW_range === 0 || rightSEW_range === 0) &&
              (leftAKH_range === 0 || rightAKH_range === 0)
            ) {
              display("Pose perfect");
            } else {
              if (leftKHH_range === -1 || rightKHH_range === -1) {
                display("Join you left leg");
              } else if (leftKHH_range === 1 || rightKHH_range === 1) {
                display("Join your right leg");
              }

              if (leftAKH_range === -1 || rightAKH_range === -1) {
                display("Bring your ankles closer to your knees");
              } else if (leftAKH_range === 1 || rightAKH_range === 1) {
                display("Spread your ankles slightly");
              }

              if (leftWEH_range === -1 || rightWEH_range === -1) {
                display("Relax your arms");
              } else if (leftWEH_range === 1 || rightWEH_range === 1) {
                display("Lower your arms slightly");
              }

              if (leftSEW_range === -1 || rightSEW_range === -1) {
                display("Spread your arms slightly");
              } else if (leftSEW_range === 1 || rightSEW_range === 1) {
                display("Raise your arms slightly");
              }
            }
            break;
          ...
          case "GODDESS":
            ...
            if (
              ...
              if (leftESH_range == -1) {
                ...
                display("Lower your left upper limb a little bit");
              } else if (rightESH_range == 1) {
