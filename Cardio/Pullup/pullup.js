...
function gotResult(error, results){
 ...
 if (results[0].confidence > 0.8) {
   ...
   if(pose.rightWrist.y<pose.nose.y-40 && pose.leftWrist.y<pose.nose.y-40){
      poseLabel="down";
    ...
    else if(pose.rightWrist.y<(pose.rightShoulder.y+40)&& pose.rightWrist.y>(pose.rightShoulder.y-40))
    {
      if(pose.leftWrist.y<(pose.leftShoulder.y+40)&& pose.leftWrist.y>(pose.leftShoulder.y-40)){
        poseLabel="up";
      if(poseLabel==poseLabel2){
        counter=counter+1;
